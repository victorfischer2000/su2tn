...
def reverse_right_most_leg(su2tensor, reverseLeg):
    """
    ...
    (newFusionTree, newFusionTreeDirections, reverseNode, reverseNodeDirection,
     newChargeSectors, newNameOrdering, use_fFactor) = (
        reverse_leg_get_newFusionTree(reverseLeg=reverseLeg,
...
def reverse_leg_get_newFusionTree(reverseLeg, oldFusionTree, oldFusionTreeDirections,
    ...
    if reverseNodeDirection == -1:
        if reverseNode[2] == reverseLeg:
            ...
            newFusionTree.append([numberOfInternalEdges+1, reverseNode[2], 0])
            ...
            newFusionTree[reverseNode_idx] = [reverseNode[0], reverseNode[2], reverseNode[1]]
        ...
        if reverseNode[0] == reverseLeg:
            ...
            newFusionTree.append([0, numberOfInternalEdges+1, reverseNode[0]])
            newFusionTreeDirections.append(+1)
...
def reverse_leg_add_internalEdgeToChargeSectors(chargeSectors, nameOrdering, reversedLeg, numberOfInternalEdges):
    ...
    nameOrdering = nameOrdering[:numberOfInternalEdges] + [numberOfInternalEdges+1] + nameOrdering[numberOfInternalEdges:]
...
def F_factor_splittingToFusion(jA, jB, jC):
    """
    Return the F-factor F^(jd, je)_(ja, jb, jc, jabc)
    """
    return F_factor(jA=jC, jB=jC, jC=jB, jABC=jB, jd=0, je=jA)


def F_factor_fusionToSplitting(jA, jB, jC):
    """
    Return the F-factor F^(jd, je)_(ja, jb, jc, jabc)
    """
    return F_factor(jA=jA, jB=jB, jC=jB, jABC=jA, jd=jC, je=0)
...
def reverse_leg_updateDegeneracyTensors(reverseNode, reverseNodeDirection, listOfDegeneracyTensors,
    ...
    for idx, (degeneracyTensor, chargeSector) in enumerate(zip(listOfDegeneracyTensors, listOfChargeSectors)):
        ...
        if reverseNodeDirection == -1:
            if use_fFactor:
                F_factor = F_factor_fusionToSplitting(jA=ja, jB=jb, jC=jc)
                factor = (-1) ** (2*jb) * np.sqrt(2*jb + 1) * F_factor
            else:
                factor = np.sqrt(2 * jc + 1)

        elif reverseNodeDirection == 1:
            if use_fFactor:
                F_factor = F_factor_splittingToFusion(jA=ja, jB=jb, jC=jc)
                factor = np.sqrt(2 * jc + 1) * F_factor
            else:
...
if __name__ == "__main__":
    ...
    reverse_right_most_leg(su2tensor=su2tensor, reverseLeg=-3)

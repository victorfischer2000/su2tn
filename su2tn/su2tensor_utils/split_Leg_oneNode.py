...
def split_leg_oneNode(su2tensor, splitLegName):
...
def split_legs_build_fusionTree(su2tensor, splitLegName, splitLegEntry):
    ...
    other_leg = [leg for leg in su2tensor.fusionTree[0] if leg not in [0, splitLegName]]
    assert len(other_leg) == 1
    other_leg = other_leg[0]
    splitNode = su2tensor.fusionTree[0]
    splitNodeDirection = su2tensor.fusionTreeDirections[0]
    ...
    if splitNodeDirection == -1:
        if splitNode[2] == splitLegName:
            newFusionTree = [[other_leg, irrep2Name, irrep1Name]]
